package HomeWork;
//ADD, 78778-784-565-454, Зов предков, Джек Лондон, 2011, 461.33
...
public class LibraryERP {
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            switch (command) {
                ...
                case "GET_BY_AUTHOR":
                    getBooksByAuthor(tokens[1]);
                    break;

                case "GET_BY_YEAR":
                    getBooksByYear(Integer.parseInt(tokens[1]));
                    break;
    ...
    private static void getBooksByYear(int year) {
        for (int i = 0; i < countBooks; i++) {
            if (years[i] == year) {
                System.out.println(String.join(", ", authors[i], titles[i], authors[i], String.valueOf(years[i]), String.valueOf(prices[i])));
